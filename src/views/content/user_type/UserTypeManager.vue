<template>
  <div>
    ...
    <el-card class="user_type_list">
      ...
      <el-table border stripe :data="userTypeList">
        ...
        <el-table-column label="操作" width="180px">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="showUserTypeDetail(scope.row)">详情</el-button>
            <el-button type="danger" size="mini" icon="el-icon-delete" @click="deleteUserTypeById(scope.row.u_type)">
              删除
            </el-button>
          </template>
        </el-table-column>
      ...
      <el-row>
        <el-col align="center">
          <el-button type="primary" size="medium" @click="addUserTypeDialogVisible = true">添加用户类型</el-button>
        </el-col>
      </el-row>
    </el-card>
    ...
    <el-dialog class="user_type_detail" :title="'用户类型名：' + userTypeDetail.u_type_name" width="650px"
      ...
      <!-- 底部区域 -->
      <span slot="footer">
        <el-button type="primary" @click="saveUserTypeDetail">保存</el-button>
    ...
    <el-dialog title="添加用户类型" width="650px"
      ...
      <el-form size="mini" label-width="120px" ref="addUserTypeFormRef" :model="addUserTypeInfo"
               :rules="addUserTypeFormRules">
        <el-form-item label="用户类型名称" prop="u_type_name">
          <el-input v-model="addUserTypeInfo.u_type_name"></el-input>
        </el-form-item>
      </el-form>
      <!-- 底部区域 -->
      <span slot="footer">
  ...
  export default {
    ...
    data() {
      return {
        ...
        //用户权限功能列表
        userPowerList: [
          {id: 1, desc: '系统基本权限', power: '登录、查询个人信息、修改个人信息'},
    ...
    methods: {
      ...
      async deleteUserTypeById(u_type) {
        //弹框询问用户是否保存修改
        const confirmResult = await this.$confirm('此操作将永久删除此用户类型，该类型的用户将变为普通类型的用户，是否继续？', '提示', {
      ...
      //点击按钮发起更新用户类型权限请求
      async saveUserTypeDetail() {
        //弹框询问用户是否保存修改
        const confirmResult = await this.$confirm('此操作将提交修改用户类型权限，是否继续？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).catch(err => err);
